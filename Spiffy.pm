...
@ISA         = qw (Exporter);
@EXPORT      = ();
@EXPORT_OK   = qw( ProG chk_ProG format_STA parse_ft_in slope_camber ) ;
%EXPORT_TAGS = ( all   => [ qw( &ProG &chk_ProG &format_STA &parse_ft_in &slope_camber ) ],
	             parse => [ qw ( &parse_ft_in ) ]    );
