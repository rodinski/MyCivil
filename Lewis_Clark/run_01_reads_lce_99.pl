...
my $filename;
#$fileneam="./../LewisClark/output/lce_12684.orh";
#$filename="./../LewisClark/output/lce_99_684.orh";
$filename="./../LewisClark/out_to_PW/lce_99213.orh";
...
my $offset;
	...
	unless ( defined $pt &&   $pt =~ /^[BG]?[AB]?\d+$/  ) {next};
	...
	if( defined $girder->{$girder_numb}{$nth}{'pt'} ) { 
			print "\ngn=$girder_numb, nth=$nth  $line[0]\nThis pt is already defined.\nLine number=$.\n\n";
			print (join "\n", grep /^G/, keys $girder);
			exit;
		}

    $girder->{$girder_numb}{$nth}{'pt'} = $pt;
	$girder->{$girder_numb}{$nth}{'sta'} = $loc;
	$girder->{$girder_numb}{$nth}{'offset'} = $y;
	$girder->{$girder_numb}{$nth}{'n'} = $n;
	$girder->{$girder_numb}{$nth}{'e'} = $e;
	$girder->{$girder_numb}{$nth}{'f_sta'} = format_STA($loc);
	$girder->{$girder_numb}{$nth}{'ProG'} = nearest(0.001,ProG($loc));

	my $se_c = el_from_left($y, find_sec($loc))- el_from_left(0, find_sec($loc));
	$girder->{$girder_numb}{$nth}{'se_c'} = nearest(0.001,$se_c);
#    printf "%6d\t%8.2f\t%8.2f\t%8.2f\t%8.3f", $pt,$loc, $y, ProG($loc),$se_c	;
	...
	printf "\t%8.3f\n", ProG($loc) + $se_c; 
	$girder->{$girder_numb}{$nth}{'deck'} = nearest(0.001,ProG($loc) + $se_c) ;
#	printf "xy=%s,%s,%s     ",$n,$e, nearest(0.01,ProG($loc)+$se_c);
...
DumpFile('ps_girder_elevation.yaml',$girder);
 ...
 say " ";
 say " ";
 for my $loc  (21077.96, 11077.78 ) {
	 say " ";
	    my $y;
	    if ($loc > 11100) {$y=16.82}
		else {$y=50.30}
        say  "Sta $loc  -> ",  ( join " ", find_sec($loc) );
 		say "offset equals = $y";
